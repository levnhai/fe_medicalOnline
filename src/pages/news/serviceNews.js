...
const ServiceNews = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState('dich-vu');
  const navigate = useNavigate();

  useEffect(() => {
    const fetchServiceNews = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/news/category/${selectedCategory}`);
        setServiceNews(response.data);
      } catch (error) {
        console.error('Error fetching service news:', error);
      }
    };
      ...
      {serviceNews.length > 0 && (
        <div className={cx('latestNewsMain', 'md:col-span-2', 'bg-white', 'rounded-lg', 'shadow-md', 'overflow-hidden', 'hover:shadow-xl', 'transition-shadow', 'duration-300')}>
          <a href="#" className="block">
            <img src={serviceNews[0].imageUrl} alt={serviceNews[0].title} className="w-full h-64 object-cover" />
            <div className="p-4">
              <div className="flex items-center space-x-1">
                <FaNewspaper />
                <p className="text-gray-600 text-s italic">{serviceNews[0].category?.name}</p>
              </div>
              <h2 className="text-4xl font-bold mb-2">{serviceNews[0].title}</h2>
              <p className={cx('text-gray-600', 'mb-2', 'lineClamp3')}>{serviceNews[0].content}</p>
              <p className="text-gray-600 mb-2">{new Date(serviceNews[0].createdAt).toLocaleDateString()}</p>
            </div>
          </a>
        </div>
      )}
      <div className="md:col-span-1 space-y-4">
        {serviceNews.slice(1, 6).map((item) => (
  ...
  return (
    <div className={cx('newsContainer', 'bg-gray-100', 'min-h-screen', 'py-10')}>
      <div className="max-w-7xl mx-auto px-4">
        ...
        {renderLatestNews()}
        <CategoryNews title="Tin Dịch Vụ" news={serviceNews.slice(6)} />
      </div>
    </div>
  );
};
